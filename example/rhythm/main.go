...
func main() {
	...
	cmb := snd.NewComb(snd.Decibel(-6).Amp(), 11*time.Millisecond, snd.NewMixer(dly, tap0, tap1, tap2))
	reverb := snd.NewLowPass(2000, cmb)

	master.Append(mixgain, reverb)

	miniaudio.Start(gain)
